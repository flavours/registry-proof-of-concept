...
class Form(forms.BaseForm):
    languages = forms.CharField(
        "Languages", required=True, initial='["en", "de"]'
    )
    ...
    def domain_settings(self, data, settings, env):
        ...
        permanent_redirect = boolean_ish(
            env("ALDRYN_SITES_REDIRECT_PERMANENT", False)
        )
    ...
    def security_settings(self, data, settings, env):
        ...
        s["SECURE_CONTENT_TYPE_NOSNIFF"] = env(
            "SECURE_CONTENT_TYPE_NOSNIFF", False
        )
        s["SECURE_BROWSER_XSS_FILTER"] = env(
    ...
    def server_settings(self, settings, env):
        ...
        settings["DJANGO_WEB_MAX_REQUESTS"] = env(
    ...
    def i18n_settings(self, data, settings, env):
        ...
        def language_codes_to_tuple(codes):
            return [
                (code, settings["ALL_LANGUAGES_DICT"][code]) for code in codes
            ]
